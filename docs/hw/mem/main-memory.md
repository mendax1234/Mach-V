...
In Nexys 4 DDR FPGA, there are a certain number of Block RAM (BRAM) resources available. These Block RAMs are read **synchronously**. The easiest way to implement a Block RAM in Verilog is to use explicitly the synchronous read style.
...
An example of Single-Port Block RAM (Read First) implementation is shown below:
...
This is useful for the [superscalar structure](../uarch/superscalar.md) implementation of Mach-V and it is also very simple to make the dual-port version Block BRAM.
