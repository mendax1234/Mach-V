...
module Wrapper #(
    ...
    localparam IROM_DEPTH_BITS = 15;  //9 for 128 words. Change if your program is larger.
    localparam DMEM_DEPTH_BITS = 14;  //9 for 128 words. Change if your data (const+variable) is larger, and in C/Assembly to set STACK_INIT appropriately.
    ...
    always @(*) begin
        ...
        if (ComputeResult[31:DMEM_DEPTH_BITS] == DMEM_BASE[31:DMEM_DEPTH_BITS]) dec_DMEM <= 1'b1;
        else if (ComputeResult[31:MMIO_DEPTH_BITS] == MMIO_BASE[31:MMIO_DEPTH_BITS]) begin
            case (ComputeResult[MMIO_DEPTH_BITS-1:2])
        ...
        if (MemWrite) begin
            for (i = 0; i < NUM_COL; i = i + 1) begin
                if (MemWrite_out[i]) begin
                    if (dec_DMEM) begin
                        DMEM[ComputeResult[DMEM_DEPTH_BITS-1:2]][i*COL_WIDTH +: COL_WIDTH] <= WriteData_out[i*COL_WIDTH +: COL_WIDTH];
                    end
                end
            end
        end
        //ReadData_DMEM <= DMEM[ComputeResult[DMEM_DEPTH_BITS-1:2]] ; //Uncomment only if only using synch read for memory
    end
    ...
    //----------------------------------------------------------------
    always @(*) begin
        ReadData_DMEM <= DMEM[ComputeResult[DMEM_DEPTH_BITS-1:2]];  // async read
    end
    ...
    always @(posedge CLK) begin
                ...
                if (MemWrite_out[0] && dec_OLED_DATA) begin
                    ...
                    if (autoadvance_col)
                        if (OLED_Col >= 95) begin  // >= as software could write >95
                            OLED_Col <= 7'd0;
                            if (OLED_Row == 63)
                                OLED_Row <= 6'd0;	// this is redundant, as it will roll over to 0 anyway. Leaving it for comprehensibility, no h/w cost 
                            else OLED_Row <= OLED_Row + 1;
                        end else OLED_Col <= OLED_Col + 1;
                end
            end
			...
			begin
                if (MemWrite_out[0] && dec_OLED_DATA) begin
                    ...
                    if (autoadvance_row)
                        ...
                        end else OLED_Row <= OLED_Row + 1;
                end
            end
            default: begin  //vary_pix_data_mode
        ...
        case (OLED_ctrl_reg[7:4])
            ...
            if (MemWrite_out[1] && MemWrite_out[0] && dec_OLED_DATA)
                OLED_Data <= {
                    WriteData_out[15:11], 3'd0, WriteData_out[10:5], 2'd0, WriteData_out[4:0], 3'd0
                };
            4'b0010: begin  // 24-bit colour mode -  byte, half-word, and whole word accessible
                if (MemWrite_out[2] && dec_OLED_DATA) OLED_Data[23:16] <= WriteData_out[23:16];
                if (MemWrite_out[1] && dec_OLED_DATA) OLED_Data[15:8] <= WriteData_out[15:8];
                if (MemWrite_out[0] && dec_OLED_DATA) OLED_Data[7:0] <= WriteData_out[7:0];
            end
            ...
            if (MemWrite_out[0] && dec_OLED_DATA)
                OLED_Data <= {
                    WriteData_out[7:5], 5'd0, WriteData_out[4:2], 5'd0, WriteData_out[1:0], 6'd0
                };
        endcase

        if (MemWrite_out[0] && dec_OLED_CTRL) OLED_ctrl_reg <= WriteData_out[7:0];
        if (MemWrite_out[0] && dec_OLED_ROW) OLED_Row <= WriteData_out[5:0];
        if (MemWrite_out[0] && dec_OLED_COL) OLED_Col <= WriteData_out[6:0];
    ...
    always @(posedge CLK) begin
        UART_TX_valid <= 1'b0;
        UART_RX_ack   <= 1'b0;
        if (MemWrite_out[0] && dec_UART_TX && UART_TX_ready) begin
            UART_TX <= WriteData_out[7:0];
            UART_TX_valid <= 1'b1;
        end
        if (MemRead && dec_UART_RX && UART_RX_valid) UART_RX_ack <= 1'b1;
    end
